...
function dictation(failure = false) {
  ...
  const useVoice = loadApp('app/hooks/useVoice.tsx', {
    ...
    'api/voice': { uploadVoiceFile: async (uri) => { calls++; assert.equal(uri, 'file:///test.m4a'); if (failure) throw new Error('transcription failed'); return 'Test dictation'; } },
  }).default;
